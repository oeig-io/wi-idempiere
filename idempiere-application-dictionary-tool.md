---
name: idempiere-application-dictionary-tool
...
INSERT INTO ad_tablename (
    ...
    ad_tablename_uu           -- generate_uuid() or pre-minted
) VALUES (...);
```

> 🔗 **Reference:** When an entity will be referenced by other scripts, use a pre-minted UUID instead of `generate_uuid()`. See [idempiere-pre-minted-uuid-tool.md](idempiere-pre-minted-uuid-tool.md).
...
SELECT 0, 0,
    ...
    ad_role_id, 'Y', now(), 100, now(), 100, generate_uuid()
FROM ad_role
WHERE isactive = 'Y'
  AND NOT EXISTS (
...
INSERT INTO ad_field (
    ...
    1, 'N', 590, 0, 'N', 'N', 'N', 'U', generate_uuid(),
    'Y', 'N', 0, 2, 2
);
```
